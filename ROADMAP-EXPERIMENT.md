...
- [x] **CDN cache directive on `/privacy`** (perf, ops, SEO). page is fully static (no per-user state, no auth, no query params), so a `+page.server.ts` `load` now sets `Cache-Control: public, max-age=3600, s-maxage=86400, stale-while-revalidate=604800`. nearly every privacy hit serves from vercel's edge cache; the function only runs on a true miss. live curl confirms the header. trade-off accepted: legal copy edits propagate within minutes via the deploy invalidation, with up to 7d soft stale on the swr tail.
