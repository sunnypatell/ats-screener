...
- [ ] **sample `scan_logs` writes to 1 in N** (engineering, ops, growth). today every scan double-writes (history + scan_logs). at 50k users with even modest weekly use, this blows past firestore spark's 20k writes/day. sample at 10% with a config knob; admin observability degrades gracefully. test: unit test the sampler is deterministic per-uid+timestamp; integration: assert sample rate over 1000 simulated scans.
- [ ] **skip `loadHistory()` reload after save** (engineering, perf). `saveToHistory` currently appends one document and then re-reads the whole window. mutate `scanHistory` locally instead and only reload on cold-start or explicit refresh. cuts firestore reads per scan in half. test: vitest covers the local mutation path; smoke: scan twice, observe history list updates without a network round trip.
...
- [ ] **add a `Cache-Control: public, max-age=300` to `/sitemap.xml` and `/robots.txt`** (perf, ops). cheap CDN caching for crawler endpoints. test: curl the headers.

## priority 2, security

- [ ] **resume-content XSS audit** (security, engineering). every place resume text or LLM-derived strings render in svelte must use `{value}` interpolation, never `{@html}`. grep, document, lock down. test: grep produces a clean allowlist; add a unit test that round-trips a payload like `<img src=x onerror=...>` through the pipeline and asserts it shows up as text.
...
- [ ] **CSP report dedupe and rate cap** (security, ops). `/api/csp-report` currently `console.warn`s every report. a single misconfigured browser extension can spam logs. dedupe by `(directive, blocked-uri)` with a 5-minute window and a 100/min cap. test: post the same report 200 times, only the first surfaces.
...
- [ ] **`/api/version` plus frontend deploy-detection** (UX, ops). returns commit sha at build time. frontend polls hourly; if mismatch, surfaces a "new version available, refresh" toast. test: curl the endpoint, mock a mismatch in the store, confirm toast renders.
