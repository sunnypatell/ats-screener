...
- **the same feed served one origin's links to every host.** `buildFeed` memoised the generated xml in a single module-level variable while embedding the request origin in both the channel `<link>` and the atom self-link, so whichever hostname happened to warm a serverless instance won for every subsequent request on it. a single deployment answers on both the project domain and its generated deployment url, so the canonical feed could end up advertising the deployment url to feed readers. the memo is now keyed by origin, which fixes the bleed while preserving the deliberate dynamic-origin behaviour that `sitemap.xml`, `robots.txt` and `llms.txt` also rely on. `releases.xml` was the only endpoint holding module-level state, so the others were never affected.
