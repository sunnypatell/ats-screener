...
- **the same feed served one origin's links to every host.** `buildFeed` memoised the whole generated xml in a single module-level variable while embedding the request origin in both the channel `<link>` and the atom self-link, so whichever hostname happened to warm a serverless instance won for every subsequent request on it. a single deployment answers on both the project domain and its generated deployment url, so the canonical feed could end up advertising the deployment url to feed readers. only those two links actually depend on origin, so the memo now covers just the origin-independent work (parsing the changelog and rendering the `<item>` blocks) and the small wrapper is interpolated per request. that keeps the expensive part cached without holding a feed per hostname, and preserves the deliberate dynamic-origin behaviour `sitemap.xml`, `robots.txt` and `llms.txt` also rely on. `releases.xml` was the only endpoint holding module-level state, so the others were never affected.
